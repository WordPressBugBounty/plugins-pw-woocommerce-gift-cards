...
final class PW_Gift_Cards_Admin {
    ...
    function __construct() {
        ...
        $this->settings = array(
            ...
            array(
                ...
                'desc'    => __( 'Allow customers to purchase gift cards using another gift card. Default: Unchecked.', 'pw-woocommerce-gift-cards' ),
        ...
        add_action( 'woocommerce_after_product_attribute_settings', array( $this, 'woocommerce_after_product_attribute_settings' ), 10, 2 );
        ...
        add_action( 'wp_ajax_pw-gift-cards-reset_design', array( $this, 'ajax_reset_design' ) );
        ...
        if ( !empty( $hook ) && substr( $hook, -strlen( 'pw-gift-cards' ) ) === 'pw-gift-cards' ) {
            ...
            wp_localize_script( 'pw-gift-cards-admin', 'pwgc', array(
                ...
                'i18n' => array(
                    ...
                    'reset_design_prompt' => __( 'Reset to the default Email Design?', 'pw-woocommerce-gift-cards' ),
                ),
                'nonces' => array(
                    ...
                    'reset_design' => wp_create_nonce( 'pw-gift-cards-reset-design' ),
    ...
    function woocommerce_after_product_attribute_settings( $attribute, $i ) {
        ?>
        <script type='text/javascript'>
            jQuery( function() {
                jQuery('.enable_variation').addClass('show_if_<?php echo PWGC_PRODUCT_TYPE_SLUG; ?>');
                var product_type = jQuery( 'select#product-type' ).val();
                if (product_type == '<?php echo PWGC_PRODUCT_TYPE_SLUG; ?>') {
                    jQuery('.enable_variation').show();
                }
            });
        </script>
        <?php
    ...
    function ajax_search() {
        ...
        if ( PWGC_UTF8_SEARCH ) {
            $sql = $wpdb->prepare( "
                ...
                LEFT JOIN
                    `{$wpdb->prefix}woocommerce_order_itemmeta` AS order_itemmeta_number ON (order_itemmeta_number.meta_key = CONVERT('pw_gift_card_number' USING utf8) AND CONVERT(order_itemmeta_number.meta_value USING utf8) = CONVERT(gift_card.number USING utf8) )
                LEFT JOIN
    ...
    function ajax_save_settings() {
        ...
        $redirect = ( get_option( 'hide_pimwick_menu', 'no' ) != ( isset( $form['hide_pimwick_menu'] ) ? 'yes' : 'no' ) );
        ...
        if ( $redirect ) {
            $redirect = pwgc_admin_url( 'settings' );
        }

        wp_send_json_success( array( 'html' => $html, 'redirectUrl' => $redirect ) );
    }
    ...
    function ajax_reset_design() {
        check_ajax_referer( 'pw-gift-cards-reset-design', 'security' );

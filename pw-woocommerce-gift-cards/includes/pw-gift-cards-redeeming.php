...
final class PW_Gift_Cards_Redeeming {
    ...
    function __construct() {
        ...
        if ( class_exists( 'Mollie_WC_Plugin' ) ) {
            $redeem_hook_priority = 1;
        }

        // WooCommerce PayPal Payments by WooCommerce (only if Taxjar is not enabled)
        if ( class_exists( 'WooCommerce\PayPalCommerce\ApiClient\Factory\PurchaseUnitFactory' ) && !class_exists( 'WC_Taxjar' ) ) {
            $redeem_hook_priority = 9;
        }
        ...
        // Fixes issues with Discount Rules for WooCommerce by Flycart
        if ( defined( 'WDR_VERSION' ) ) {
            $redeem_hook_priority = 1;
        }

        $redeem_hook_priority = apply_filters( 'pwgc_redeem_hook_priority', $redeem_hook_priority );
    ...
    function woocommerce_after_calculate_totals( $cart ) {
        ...
        if ( property_exists( $cart, 'pwgc_calculated_total' ) ) {
            ...
            //
            if ( !defined( 'PPOM_VERSION' ) && !defined( 'WCML_VERSION' ) && !defined( 'WDR_VERSION' ) ) {
        ...
        // Allow third party plugins to modify the session data before it is stored.
        $session_data = apply_filters( 'pwgc_redeeming_session_data', $session_data, $cart );

        // Allow third party plugins to recalculate the total amount of gift cards redeemed.
        $gift_card_total = apply_filters( 'pwgc_redeeming_gift_card_total', $gift_card_total, $session_data, $cart );
    ...
    function woocommerce_order_status_preordered( $order_id ) {
        $order = wc_get_order( $order_id );
        $this->debit_gift_cards( $order_id, $order, "order_id: $order_id preordered" );
    }
    ...
    function woocommerce_applied_coupon( $coupon_code ) {
        if ( true === $this->is_gift_card_code( $coupon_code ) ) {
            ...
            if ( $result === true ) {
                ...
                wc_add_notice( __( 'Error', 'pw-woocommerce-gift-cards' ) . ': ' . $result, 'error' );
            }
        }
    }
    ...
    function yith_pdf_invoice_before_total( $current_order ) {
        foreach( $current_order->get_items( 'pw_gift_card' ) as $line ) {
            $gift_card_total = 0;

            $amount = apply_filters( 'pwgc_to_order_currency', floatval( $line->get_amount() ) * -1, $current_order );
            ...
            if ( !empty( $gift_card_total ) ) {
                ?>
                <tr>
                    <td class="left-content column-product"><?php echo __( 'PW Gift Card', 'pw-woocommerce-gift-cards' ) . ' ' . $line->get_card_number(); ?></td>
                    <td class="right-content column-total"><?php echo $gift_card_total; ?></td>
                </tr>
                <?php
            }
        }
